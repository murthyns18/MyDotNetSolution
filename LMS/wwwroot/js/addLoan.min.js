function loadUsers(){$.ajax({url:apiURL+"User/UserList",type:"GET",data:{userID:-1},success:function(n){$("#ddlUser").empty().append(`<option value="">-- Select User --</option>`);n.forEach(n=>{$("#ddlUser").append(`<option value="${n.userID}">${n.userName}</option>`)})}})}function loadPublishers(){$.ajax({url:apiURL+"Publisher/PublisherList",type:"GET",data:{publisherID:-1},success:function(n){$("#ddlPublisher").empty().append(`<option value="">-- Select Publisher --</option>`);n.forEach(n=>{$("#ddlPublisher").append(`<option value="${n.publisherID}">${n.publisherName}</option>`)})}})}function addBook(){const n=$("#ddlBook").val(),t=$("#ddlBook option:selected").text(),i=$("#ddlPublisher option:selected").text();if(!n){App.alert("Please select a book");return}if(selectedBooks.length>=4){App.alert("Maximum 4 books allowed");return}if(selectedBooks.some(t=>t.bookId==n)){App.alert("Book already added");return}selectedBooks.push({bookId:parseInt(n),bookName:t,publisherName:i});renderSelectedBooks()}function removeBook(n){selectedBooks=selectedBooks.filter(t=>t.bookId!==n);renderSelectedBooks()}function renderSelectedBooks(){const n=$("#tblSelectedBooks tbody");if(n.empty(),selectedBooks.length===0){n.append(`
            <tr>
