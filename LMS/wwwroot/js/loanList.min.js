function actionFormatter(n,t,i){let r=`
    ...
    `;return i.status==="Active"&&(r+=`
        ...
        `),r+`</div>`}function statusFormatter(n){return isExport?n:n==="Closed"?"<span class='badge bg-secondary'>Closed<\/span>":"<span class='badge bg-success'>Active<\/span>"}function reloadLoanGrid(){$("#loanGrid").jqGrid("setGridParam",{datatype:"json",page:1}).trigger("reloadGrid")}function dateFormatter(n){if(!n)return isExport?"Not Returned":"<span class='text-danger fw-semibold'>Not Returned<\/span>";const t=new Date(n);return isNaN(t)?isExport?"Not Returned":"<span class='text-danger fw-semibold'>Not Returned<\/span>":t.toLocaleDateString("en-GB")}function deleteLoan(n,t){confirm(`Are you sure you want to delete this loan "${t}?"`,function(){$.ajax({url:"/Loan/DeleteLoan",type:"POST",data:{loanId:n,__RequestVerificationToken:$('input[name="__RequestVerificationToken"]').val()},success:function(t){$("#loanGrid").jqGrid("delRowData",n);App.alert(t.message)},error:function(){App.alert("Delete failed")}})})}function returnLoan(n,t){confirm(`Return loan for "${t}"?`,function(){$.ajax({url:"/Loan/ReturnLoan",type:"POST",data:{loanId:n,__RequestVerificationToken:$('input[name="__RequestVerificationToken"]').val()},success:function(n){App.alert(n.message);reloadLoanGrid()},error:function(){App.alert("Return failed")}})})}function openAddLoanModal(){selectedBooks=[];$("#loanDetailsContainer").empty();$("#tblSelectedBooks tbody").html(`
        <tr>
